...
public class UplwServlet extends HttpServlet{
	...
	throws IOException{
		...
		try {
			...
			long c = f1 - f2;
		    if(c<0){
		    	request.getSession().setAttribute("msg", "已经超过论文最后提交日期，你不能提交论文！！！");
		...
		String sdepart=stu.getSdepart();
		 xuanti xt=xuan.searchBySid(sid);
		 if(xt==null){
	...
	try {
			lw.add(name,sid,sname,sdepart,stel,tname);	
			request.getSession().setAttribute("msg", "恭喜论文提交成功");
		} catch (SQLException e) {
			request.getSession().setAttribute("msg", "请不要重复提交论文");
		}
		response.sendRedirect("student/msg.jsp");
	}
}	
